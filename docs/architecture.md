...
- Implementations: `SimBroker` (backtest + paper), `LiveBroker` (Dhan / Fyers / Angel One / Kite).
...
- Persists 1-min candles to disk (Parquet) for later backtests.
...
├── src/agent/
│   ├── broker/                # base.py, sim.py, dhan.py | fyers.py | ...
│   ├── data/                  # candles.py, indicators.py, staleness.py
│   ├── news/                  # client.py
...
├── tests/
├── data/                      # candles (parquet), journal.db  — gitignored
└── docs/
```
...
- 1-min data, at least 12 months, covering trending and choppy periods.
...
- Walk-forward: tune on months 1–8, validate on 9–12. No re-tuning on the validation set.
...
|---|---|---|
| **0. Spec & data** | This doc finalized; 1-min historical data source chosen; broker chosen | 12 months of clean 1-min data loadable |
...
## 12. Open decisions
- Broker: Dhan / Fyers / Angel One (free) vs Kite Connect (paid) — pick based on WebSocket quality, bracket-order support, and historical 1-min data access.
- News source for Indian equities (coverage, timestamp accuracy, cost).
- Historical 1-min data source for backtests (broker history depth vs a paid vendor).
